...
export function autoScale(minValue, maxValue) {
	let scl = Scaler.autoScale(minValue, maxValue) 
...
export function lineGraph( options, pData)  {
	let cv=$canvas()
	let div=$div({}, cv)
...
export function cumulatedBarGraph( options, pData ) {
	let cv=$canvas()
	let div=$div({}, cv)
	let sensitiveArea=[]
	let i,j
	...
	let asdata = autoScale(minValue, maxValue)
	...
	for (i in data) {
		...
		for (j=1; j<data[i].length && j<= pData.bars.length; j++) {
			...
			let x1= (i*hScale+barOffset ) 
			let y1= barSum * vScale 
			let w1=(barWidth-2) 
			let h1=((data[i][j])*vScale) 
	...
	if (withLinks) {
		...
		for ( let i in pData.bars ) {
			let l="<div style=\"display: inline-block;height:"+ txtSize +"px; width: "+txtSize*2 + "px; background-color: "+ colors[i] +"\"></div> "
			l+=pData.bars[i]
	...
	let getAreaId = function(ev) {
		...
		x-= o.left; 
		y-= o.top; 				
		for(let i in sensitiveArea){
			let pt=sensitiveArea[i]
...
export function barGraph( options, pData ) {
	let cv=$canvas()
	let div=$div({}, cv)
	...
	for (i in data) {
		for (j=1; j<data[i].length && j<= pData.bars.length; j++) {
			ctx.fillStyle=colors[j-1]
			let x1= (i*hScale+barOffset +barWidth/(pData.bars.length)*(j-1)) |0
			let y1=0
			let w1=(barWidth/(pData.bars.length)-2) |0
			let h1=(data[i][j]*vScale) |0
			ctx.fillRect(x1,y1,w1,h1)
			if (onclick) sensitiveArea.push([ox+x1,oy+y1,ox+x1+w1,oy+y1+h1,data[i][0],pData.bars[j-1]]) 
		}
		ctx.fillStyle='black'
...
export function pieChart( options, pData ) {
